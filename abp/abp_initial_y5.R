...
library(readr)
raw_abp_y5 <- read.delim("abp_y5.txt", header=TRUE, sep="\t")
View(raw_abp_y5)
...
raw_abp_y5$date_abp <- as.Date(raw_abp_y5$datetime)
raw_abp_y5$date_start <- as.Date(raw_abp_y5$datetime)
raw_abp_y5$date_wake <- as.Date(raw_abp_y5$datetime)


abp_check_y5_start <- right_join(raw_abp_y5, checklist, by = c("subjectid", "date_start"), all.x = TRUE)
abp_check_y5_wake <- right_join(raw_abp_y5, checklist, by = c("subjectid", "date_wake"), all.x = TRUE)

abp_check_y5_start_2 <- merge(raw_abp_y5, checklist, by = c("subjectid", "date_start"), all.x = TRUE)
abp_check_y5_wake_2 <- merge(raw_abp_y5, checklist, by = c("subjectid", "date_wake"), all.x = TRUE)


abp_check_y5_start <- filter(abp_check_y5_start, !is.na(systolic_bp))
abp_check_y5_wake <- filter(abp_check_y5_wake, !is.na(systolic_bp))

abp_check_y5_start_2 <- filter(abp_check_y5_start_2, !is.na(date_bike))
abp_check_y5_wake_2 <- filter(abp_check_y5_wake_2, !is.na(date_bike))


abp_check_y5 <- bind_rows(abp_check_y5_start, abp_check_y5_wake)

abp_check_y5_2 <- bind_rows(abp_check_y5_start_2, abp_check_y5_wake_2)

#  abp_check_y5 and abp_check_y5_2  are same data table 


#adding sleep wake 
abp_check_y5$timeA <- as.POSIXct(abp_check_y5$datetime,format = "%Y-%m-%d %H:%M", tz = "America/New_York" )

abp_check_y5$sleep_wake <- ifelse(abp_check_y5$timeA >= abp_check_y5$sleep  & 
                                    abp_check_y5$timeA <= abp_check_y5$wakeup, "sleep", "wake") 

abp_check_y5$stage [abp_check_y5$sleep_wake == "sleep"] <- "sleep"

#add baseline; created numeric times for each because that is the only way I could get it to work

abp_check_y5$morn_bike_start_nu <- as.numeric(abp_check_y5$morn_bike_start)
abp_check_y5$wakeup_nu <- as.numeric(abp_check_y5$wakeup)
abp_check_y5$timeA_nu <- as.numeric(abp_check_y5$timeA)

# subtracted an hour from start of bike ride to get begining of baseline
abp_check_y5$baseT <- abp_check_y5$morn_bike_start_nu - 3600

abp_check_y5$stage [abp_check_y5$timeA_nu < abp_check_y5$morn_bike_start_nu & 
                      abp_check_y5$timeA_nu >= abp_check_y5$baseT] <- "Baseline"


#add immed after morning ride ( one hour period); add hour to end of bike time to get end of immed period

abp_check_y5$morn_bike_end_nu <- as.numeric(abp_check_y5$morn_bike_end)
abp_check_y5$immedT <- abp_check_y5$morn_bike_end_nu + 3600

abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$morn_bike_end_nu & 
                      abp_check_y5$timeA_nu <= abp_check_y5$immedT] <- "immed"

#post1 hour after morning ride
abp_check_y5$post1T <- abp_check_y5$immedT + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$immedT & 
                      abp_check_y5$timeA_nu <= abp_check_y5$post1T] <- "post1"

#post 2 hours after morning ride
abp_check_y5$post2T <- abp_check_y5$post1T + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$post1T & 
                      abp_check_y5$timeA_nu <= abp_check_y5$post2T] <- "post2"

#post 3 hours after morning ride
abp_check_y5$post3T <- abp_check_y5$post2T + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$post2T & 
                      abp_check_y5$timeA_nu <= abp_check_y5$post3T] <- "post3"

#post 4 hours after morning ride 
abp_check_y5$post4T <- abp_check_y5$post3T + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$post3T &
                      abp_check_y5$timeA_nu <= abp_check_y5$post4T] <- "post4"



# baseline period before evening ride

abp_check_y5$eve_bike_start_nu <- as.numeric(abp_check_y5$eve_bike_start)
abp_check_y5$eve_bike_end_nu <- as.numeric(abp_check_y5$eve_bike_end)

abp_check_y5$baseT_eve <- abp_check_y5$eve_bike_start_nu - 3600

abp_check_y5$stage [abp_check_y5$timeA_nu < abp_check_y5$eve_bike_start_nu & 
                      abp_check_y5$timeA_nu >= abp_check_y5$baseT_eve] <- "Eve_Baseline"

#immed after evening ride 
abp_check_y5$immedT_eve <- abp_check_y5$eve_bike_end_nu + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$eve_bike_end_nu 
                    & abp_check_y5$timeA_nu < abp_check_y5$immedT_eve] <- "Eve_immed"

# post 1 hour after evening ride
abp_check_y5$post1T_eve <- abp_check_y5$immedT_eve + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$immedT_eve & abp_check_y5$timeA_nu <= abp_check_y5$post1T_eve] <- "Eve_post1"

#post 2 hours after evening ride
abp_check_y5$post2T_eve <- abp_check_y5$post1T_eve + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$post1T_eve & abp_check_y5$timeA_nu <= abp_check_y5$post2T_eve] <- "Eve_post2"

#post 3 hours after evening ride
abp_check_y5$post3T_eve <- abp_check_y5$post2T_eve + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$post2T_eve & abp_check_y5$timeA_nu <= abp_check_y5$post3T_eve] <- "Eve_post3"

#post 4 hours after evening ride
abp_check_y5$post4T_eve <- abp_check_y5$post3T_eve + 3600
abp_check_y5$stage [abp_check_y5$timeA_nu > abp_check_y5$post3T_eve & abp_check_y5$timeA_nu <= abp_check_y5$post4T_eve] <- "Eve_post4"
...
summary(abp_check_y5$stage=="sleep")

# 927 total sleep readings

#number of non-error sleep readings

summary(abp_check_y5$stage=="sleep" & is.na(abp_check_y5$event_code))

# 817 night readings (non-error) 

#get rid of 'sessionid' as it has incorrect session id. use 'session_id' instead

abp_check_y5$sessionid <- NULL

# subset of data with only non-error readings 

abp_check_y5_read <- filter(abp_check_y5, abp_check_y5$event_code!= "EE" | is.na(abp_check_y5$event_code))
# succussfull readings  : 8109

# add binary columns for sleep and wake to easily get sums 

abp_check_y5_read$sleep_read <- ifelse(abp_check_y5_read$sleep_wake=="sleep",1,0)
abp_check_y5_read$wake_read <- ifelse(abp_check_y5_read$sleep_wake=="wake",1,0)

abp_check_y5_read$stage [is.na(abp_check_y5_read$stage)] <- "wake"

#add binary for baseline, immed, post1, ect... 
abp_check_y5_read$base_read <- ifelse(abp_check_y5_read$stage=="Baseline",1,0)
abp_check_y5_read$immed_read <- ifelse(abp_check_y5_read$stage=="immed",1,0)
abp_check_y5_read$post1_read <- ifelse(abp_check_y5_read$stage=="post1",1,0)
abp_check_y5_read$post2_read <- ifelse(abp_check_y5_read$stage=="post2",1,0)
abp_check_y5_read$post3_read <- ifelse(abp_check_y5_read$stage=="post3",1,0)
abp_check_y5_read$post4_read <- ifelse(abp_check_y5_read$stage=="post4",1,0)
abp_check_y5_read$eve_base_read <- ifelse(abp_check_y5_read$stage=="Eve_Baseline",1,0)
abp_check_y5_read$eve_immed_read <- ifelse(abp_check_y5_read$stage=="Eve_immed",1,0)
abp_check_y5_read$eve_post1_read <- ifelse(abp_check_y5_read$stage=="Eve_post1",1,0)
abp_check_y5_read$eve_post2_read <- ifelse(abp_check_y5_read$stage=="Eve_post2",1,0)
abp_check_y5_read$eve_post3_read <- ifelse(abp_check_y5_read$stage=="Eve_post3",1,0)
abp_check_y5_read$eve_post4_read <- ifelse(abp_check_y5_read$stage=="Eve_post4",1,0)
...
ABP_summary_y5 <- abp_check_y5_read %>% group_by(subjectid, session_id) %>% dplyr::summarise(subjectid[1], session_id[1], datetime[1], length(systolic_bp), min(systolic_bp), mean(systolic_bp), max(systolic_bp), sd(systolic_bp), min(diastolic_bp), mean(diastolic_bp), max(diastolic_bp), sd(diastolic_bp), mean(mean_arterial_p), mean(heart_rate), sum(sleep_read), sum(wake_read), sum(base_read), sum(immed_read), sum(post1_read),
...
names(ABP_summary_y5)[names(ABP_summary_y5)=="sum(sleep_read)"] <- "sleep_read"

mean(ABP_summary_y5$sleep_read, na.rm = TRUE)
...
abp_check_y5_read<- filter(abp_check_y5_read, systolic_bp < 260)
abp_check_y5_read<- filter(abp_check_y5_read, systolic_bp > 70)
abp_check_y5_read<- filter(abp_check_y5_read, diastolic_bp < 150)
abp_check_y5_read<- filter(abp_check_y5_read, diastolic_bp > 40)

# table of means
abp_stages_mean <- abp_check_y5_read %>% group_by(subjectid, session_id, stage) %>% dplyr::summarise(length(systolic_bp), min(systolic_bp), mean(systolic_bp), max(systolic_bp), sd(systolic_bp), min(diastolic_bp), mean(diastolic_bp), max(diastolic_bp), sd(diastolic_bp), mean(mean_arterial_p))
...
#all waketime data
wakesleep_mean <- abp_check_y5_read %>% group_by(subjectid, session_id, sleep_wake) %>% dplyr::summarise(length(systolic_bp), min(systolic_bp), mean(systolic_bp), max(systolic_bp), sd(systolic_bp), min(diastolic_bp), mean(diastolic_bp), max(diastolic_bp), sd(diastolic_bp), mean(mean_arterial_p))
...
abp_wide_y5 <- full_join(base_means, immed_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, post1_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, post2_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, post3_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, post4_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, Ebase_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, Eimmed_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, Epost1_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, Epost2_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, Epost3_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, Epost4_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, sleep_means, by= c("subjectid", "session_id"))
abp_wide_y5 <- full_join(abp_wide_y5, wake_means, by= c("subjectid", "session_id"))
...
abp_base_read <- filter(abp_check_y5_read, base_read == 1 | eve_base_read == 1)
...
library(dplyr)
library(ggplot2)
abp_check_y5_read$time_short <- format(as.POSIXct(strptime(abp_check_y5_read$datetime, "%Y-%m-%d %H:%M", tz= "America/New_York")), format = "%H:%M")
#keeps staged in order
abp_check_y5_read$stage <- factor(abp_check_y5_read$stage, levels = c("Baseline", "immed", "post1","post2","post3","post4","Eve_Baseline", "Eve_immed", "Eve_post1","Eve_post2","Eve_post3","Eve_post4", "sleep"))

b2076 <- filter(abp_check_y5_read, subjectid == "BIKE2076")
...
#sleeptime data
abp_wide_y5$changescore_sys <- (abp_wide_y5$sys_sleep_mean * abp_wide_y5$sys_wake_mean)/abp_wide_y5$sys_wake_mean
abp_wide_y5$dnratio_sys <- abp_wide_y5$sys_sleep_mean / abp_wide_y5$sys_wake_mean
#Nocturnal decline
abp_wide_y5$nocdecline <- ((abp_wide_y5$sys_wake_mean - abp_wide_y5$sys_sleep_mean)/abp_wide_y5$sys_wake_mean) * 100
#Dippers ; Non dippers if noturnal decline <= 10%
abp_wide_y5$dippers [abp_wide_y5$nocdecline <= 10] <- "Non-dipper"
abp_wide_y5$dippers [abp_wide_y5$nocdecline > 10] <- "Dipper"

abp_wide_y5_sleep <- select(abp_wide_y5, subjectid, session_id, changescore_sys, dnratio_sys, nocdecline, dippers)

write.csv(abp_wide_y5, "abp_wide_y5.csv")
